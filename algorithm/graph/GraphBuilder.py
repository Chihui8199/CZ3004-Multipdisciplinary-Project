...
class GraphBuilder:
    ...
    def checkValidAction(self, action, current, target, x, y, new_x, new_y, dir, new_dir, rotation=False):
        traj, path_cost = Car().get_traj(x=x*5, y=y*5, z=((dir+1)*math.pi/2)%(2*math.pi), sample_rate=sample_rate, action=action)
            ...
            for x in range(3, side_len//step_count+2-3):
                ...
                for dir in range(4): # N W S E
                    ...
                    if dir == 0: # N
                        ...
                        # rotation
                        self.checkValidAction(fr, id, nodes * (y + 4) + 4 * (x + 4) + 3, x, y, x+4, y+4, dir, 3, rotation=True)
                        self.checkValidAction(fl, id, nodes * (y + 4) + 4 * (x - 4) + 1, x, y, x-4, y+4, dir, 1, rotation=True)
                        self.checkValidAction(br, id, nodes * (y - 4) + 4 * (x + 4) + 1, x, y, x+4, y-4, dir, 1, rotation=True)
                        self.checkValidAction(bl, id, nodes * (y - 4) + 4 * (x - 4) + 3, x, y, x-4, y-4, dir, 3, rotation=True)
                    ...
                    # similarly
                        ...
                        self.checkValidAction(fr, id, nodes * (y + 4) + 4 * (x - 4) + 0, x, y, x-4, y+4, dir, 0, rotation=True)
                        self.checkValidAction(fl, id, nodes * (y - 4) + 4 * (x - 4) + 2, x, y, x-4, y-4, dir, 2, rotation=True)
                        self.checkValidAction(br, id, nodes * (y + 4) + 4 * (x + 4) + 2, x, y, x+4, y+4, dir, 2, rotation=True)
                        self.checkValidAction(bl, id, nodes * (y - 4) + 4 * (x + 4) + 0, x, y, x+4, y-4, dir, 0, rotation=True)

                    elif dir == 2:
                        self.checkValidAction(forward, id, nodes * (y - 1) + 4 * x + dir, x, y, x, y-1, dir, dir)
                        self.checkValidAction(backward, id, nodes * (y + 1) + 4 * x + dir, x, y, x, y+1, dir, dir)

                        self.checkValidAction(fr, id, nodes * (y - 4) + 4 * (x - 4) + 1, x, y, x-4, y-4, dir, 1, rotation=True)
                        self.checkValidAction(fl, id, nodes * (y - 4) + 4 * (x + 4) + 3, x, y, x+4, y-4, dir, 3, rotation=True)
                        self.checkValidAction(br, id, nodes * (y + 4) + 4 * (x - 4) + 3, x, y, x-4, y+4, dir, 3, rotation=True)
                        self.checkValidAction(bl, id, nodes * (y + 4) + 4 * (x + 4) + 1, x, y, x+4, y+4, dir, 1, rotation=True)

                    elif dir == 3:
                        self.checkValidAction(forward, id, nodes * y + 4 * (x + 1) + dir, x, y, x+1, y, dir, dir)
                        self.checkValidAction(backward, id, nodes * y + 4 * (x - 1) + dir, x, y, x-1, y, dir, dir)

                        self.checkValidAction(fr, id, nodes * (y - 4) + 4 * (x + 4) + 2, x, y, x+4, y-4, dir, 2, rotation=True)
                        self.checkValidAction(fl, id, nodes * (y + 4) + 4 * (x + 4) + 0, x, y, x+4, y+4, dir, 0, rotation=True)
                        self.checkValidAction(br, id, nodes * (y - 4) + 4 * (x - 4) + 0, x, y, x-4, y-4, dir, 0, rotation=True)
                        self.checkValidAction(bl, id, nodes * (y + 4) + 4 * (x - 4) + 2, x, y, x-4, y+4, dir, 2, rotation=True)
        print("done")
